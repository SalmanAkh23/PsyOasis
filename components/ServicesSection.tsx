...
export default function ServicesSection() {
  return (
    <section id="layanan" className="py-20 relative">
      ...
      <div className="absolute top-1/2 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#709085]/10 to-transparent pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
          {services.map((s, i) => (
            <div
              key={i}
              className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-[#709085]/15 hover:border-[#4A7A96]/30 hover:bg-white/80 hover:shadow-[0_8px_30px_rgba(74,122,150,0.12)] cursor-pointer transition-all duration-300 transform hover:-translate-y-1.5 flex flex-col justify-between"
            >
              <div>
                {/* Icon wrapper */}
                <div className="w-12 h-12 rounded-xl bg-[#F7F9F6] flex items-center justify-center mb-5 border border-[#709085]/20 shadow-sm" style={{ color: s.accent }}>
                  {s.icon}
                </div>
                ...
                {/* Description */}
                <p className="text-xs md:text-sm text-[#2D3732]/60 leading-relaxed font-light mb-6">
                  {s.desc}
                </p>
              </div>
              ...
              <div className="mt-auto flex justify-end">
                <button
                  ...
                  className="px-4 py-2 bg-[#4A7A96] text-white font-semibold rounded-lg shadow-[0_4px_15px_rgba(74,122,150,0.2)] hover:bg-[#3D6B82] hover:shadow-[0_4px_20px_rgba(74,122,150,0.3)] transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 text-xs"
                >
